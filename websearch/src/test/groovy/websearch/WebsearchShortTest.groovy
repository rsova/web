...
//	@Before
	...
	void test_lat1_lon1_sortdirection1_number() {
		...
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatLat1':'20.0','WhatLon1':'-156.0','WhatSortType1':'ListPrice','WhatSortDirection1':'ASC','WhatNumber':'5']))
	...
	void test_startdate_enddate_sortype1_sortdirection1_sortype2_sortdirection2_number() {
		...
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934', 'WhatStartDate':'2014-08-10','WhatSortType1':'ListPrice','WhatSortDirection1':'ASC','WhatSortType2':'MLS','WhatSortDirection2':'DESC','WhatNumber':'5']))
		def actual = curl.execute().text
		println actual
		saveToFileIfNotExist(SRC_TEST_RESOURCES + "$METHOD/$test"+".xml", actual)
	...
	void test_startintarea_endintarea_startextarea_endextarea_number() {
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatStartIntArea':'800','WhatEndIntArea':'2000','WhatStartExtArea':'200','WhatEndExtArea':'1000', 'WhatNumber':'2']))
		def actual = curl.execute().text
		println actual
	...
	void test_condo_agent_startbed_endbed_startbath_endbath_number() {
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatCondo':['Pacific Shores'],'WhatAgent':['12496'],'WhatStartBed':'1','WhatEndBed':'2', 'WhatStartBath':'1','WhatEndBath':'2', 'WhatNumber':'2']))
		def actual = curl.execute().text
		println actual
	...
	void test_ohana_endprice_number() {
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatOhana':['Single Family'],'WhatEndPrice':'6700000','WhatNumber':'2']))
		def actual = curl.execute().text
		println actual
	...
	void test_water_startprice_number() {
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatWater':['BeachFront'],'WhatStartPrice':'3700000','WhatNumber':'2']))
		def actual = curl.execute().text
		println actual
	...
	void test_type_addressfull() {
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatPropType':['Condominium'],'WhatAddressFull':'Bay DR']))
		def actual = curl.execute().text
		println actual
		saveToFileIfNotExist(SRC_TEST_RESOURCES + "$METHOD/$test"+".xml", actual)
	...
	void test_type_mls() {
        //it's an or
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatPropType':['Condominium'],'WhatMLS':['329222', '359341']]))
		def actual = curl.execute().text
		println actual
		saveToFileIfNotExist(SRC_TEST_RESOURCES + "$METHOD/$test"+".xml", actual)
	...
	void test_type_page_number() {
		def test = getName().replaceFirst('test_','')
		curl.add("-d " + XmlRpcHelper.toXmlRpcRequest(METHOD,['dbid':'dbid1139259934','WhatPropType':['Condominium'], 'WhatPage':'1', 'WhatNumber':'2']))
		def actual = curl.execute().text
		println actual
		saveToFileIfNotExist(SRC_TEST_RESOURCES + "$METHOD/$test"+".xml", actual)
